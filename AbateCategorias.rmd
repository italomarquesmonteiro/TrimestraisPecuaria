---
title: "Abate bovino no Rio Grande do Sul 2º trimestre "
description: "Quantitativo por categoria atualizado em  06 de setembro de 2023 pelo IBGE"
author: 
    - name: "Ítalo Marques-Monteiro"
    - email: "italo.marques.monteiro@outlook.com"
    - github: https://github.com/italomarquesmonteiro
    - kaggle: https://www.kaggle.com/talomarquesmonteiro
    - linkedin: linkedin.com/in/ítalo-monteiro-0b2516144
    - lattes: http://lattes.cnpq.br/7174985368517137
    - orcid_id: https://orcid.org/0000-0003-4367-1034
    - affiliation: Smart Data  
        
date: "2023-09-11"
output: html_document
---

Informações:
[IBGE](https://www.ibge.gov.br/)
[Comunidado](https://agenciadenoticias.ibge.gov.br/agencia-sala-de-imprensa/2013-agencia-de-noticias/releases/37828-cresce-o-abate-de-bovinos-e-frangos-e-cai-o-de-suinos-no-2-tri-de-2023)
[Dados](https://sidra.ibge.gov.br/tabela/1092)
"/t/1092/n3/all/v/284/p/last%201/c12716/115236/c18/55,56,111734,111735/c12529/118225"
```{r}
library(tidyverse)
```

```{r}
ufs  <- sidrar::get_sidra(
    api = "/t/1092/n3/all/v/284/p/last%201/c12716/115236/c18/55,56,111734,111735/c12529/118225") |>
    janitor::clean_names("snake") |>
    dplyr::select(
        valor, tipo_de_rebanho_bovino, unidade_da_federacao,
            categoria = tipo_de_rebanho_bovino,
            uf = unidade_da_federacao
    ) |>
    dplyr::mutate(uf = as.factor(uf)) |>
    dplyr::glimpse()
```



Participação das UFs no abate bovino do Brasil
```{r}
per_abt_total <- ufs |>
    dplyr::group_by(uf) |>
    dplyr::summarize(
        total = sum(valor, na.rm = TRUE),
        percent = total / 9075548) |>
    dplyr::mutate(perc2 = scales::percent(total / 9075548, accuracy = .1, trim = FALSE)) |>
    dplyr::arrange(desc(perc2))


#test1 <- per_abt_total #|>
    #dplyr::mutate(
    #    color = case_when(
    #      row_number() == 1 ~ "goldenrod1",
    #      row_number() == 2 ~ "mediumpurple1",
    #      row_number() == 3 ~ "coral2",
    #      ## all others should be gray
    #      TRUE ~ "gray70"),
    #    color = forcats::as_factor(color))



sysfonts::font_add(family = "Font Awesome 6 Brands", regular = "C:/Users/italo/AppData/Local/Microsoft/Windows/Fonts/Font Awesome 6 Brands-Regular-400.otf") # nolint
showtext::showtext_auto()
showtext::showtext_opts(dpi = 300)
bg <- "white"
txt_col <- "black"
github_icon <- "&#xf09b"
linkedin_icon <- "&#xf0e1"
x_icon <- "&#xf099"
instagram_icon <- "&#xf16d"
github_username <- "italomarquesmonteiro"
linkedin_username <- "italomarquesmonteiro"
x_username <- "italommonteiro"
instagram_username <- "italo.m.m"
colors <- "grey40"
title_text <- glue::glue('Participação das UFs no <span style="color:{colors}">**abate total**</span> de bovinos') # nolint
subtitle_text <- glue::glue("")
caption_text <- glue::glue(
  "**Dados:**  IBGE [Diretoria de Pesquisas Agropecuárias, Coordenação de Agropecuária, Pesquisa Trimestral do Abate de Animais(2024)]<br>", # nolint
  "**Nota:** As unidades da federação, Amapá, Distritio Federal e Paraíba não apresentaram dados<br>",
  "**Plot:** Ítalo Marques-Monteiro <br><br>",
  "<span style='font-family:\"Font Awesome 6 Brands\"; color: black;'>{github_icon};</span> 
  <span style='color: black'>{github_username}</span><br>",
  "<span style='font-family:\"Font Awesome 6 Brands\"; color: dodgerblue4;'>{linkedin_icon};</span> 
  <span style='color: black'>{linkedin_username}</span><br>",
  "<span style='font-family:\"Font Awesome 6 Brands\"; color: steelblue;'>{x_icon};</span>
  <span style='color: black'>{x_username}</span><br>",
  "<span style='font-family:\"Font Awesome 6 Brands\"; color: red;'>{instagram_icon};</span>
  <span style='color: black'>{instagram_username}</span>"
)

part_ufs_abt_br <- per_abt_total |>
    dplyr::mutate(
        color = case_when(
          row_number() == 1 ~ "goldenrod1",
          row_number() == 2 ~ "mediumpurple1",
          row_number() == 3 ~ "coral2",
          ## all others should be gray
          TRUE ~ "gray70"),
        color = forcats::as_factor(color)
    ) |>
    ggplot(aes(x = percent, y = reorder(uf, percent))) +
        geom_col(aes(fill = color))+
        scale_fill_identity(guide = "none") +
        geom_text(mapping = aes(label = scales::percent(percent, accuracy = 0.1)),
             position = position_dodge(0.9),
             size = 3.5,
             color = "gray20",
             vjust = 0.5,
              hjust = 0.01) +
        theme_void() +
        labs(
            title = title_text,
            subtitle = "4º trimestre de 2023",
            caption = caption_text) +
        theme(
            plot.title = ggtext::element_markdown(color = "gray30", face = "bold", size = 25, hjust = 0.1),
            plot.subtitle = element_text(family = "italic", size = 15, color = "gray50", hjust = 0.1),
            plot.caption = ggtext::element_markdown(hjust = 0, margin = margin(10,0,0,0), size = 8, color = txt_col, lineheight = 1.2), # nolint
            axis.text.y = element_text(color = "gray40", size = 12, hjust = 1, family = "Fira Sans"),
            plot.margin = margin(rep(15, 4)),
            panel.background = element_rect(fill = "white", color = "white"),
            plot.background = element_rect(fill = "white")
        )

abt_ufs <- data.frame(image = ".vscode/Images/mapa_abt_ufs.png") 

part_ufs_abt_br <- part_ufs_abt_br +
    ggimage::geom_image(aes(x = 0.10, y = 10, image = image), data = abt_ufs,  size = 1)  # nolint

ggsave(".vscode/Images/part_ufs_abt_br.png", plot = part_ufs_abt_br, dpi = 300)
```



Participação de Vacas no abate nacional
```{r}
ufs_vacas <- ufs |>
    dplyr::filter(categoria == "Vacas") |>
    dplyr::mutate(uf = as.factor(uf)) |>
    dplyr::glimpse()


ufs_vacas |>
    dplyr::summarize(
        total = sum(valor, na.rm = TRUE))


prop_vaca <- ufs_vacas |>
    dplyr::group_by(uf) |>
    dplyr::summarize(
        total = sum(valor, na.rm = TRUE),
        perc = total / 2516575) |>
    dplyr::mutate(perc2 = scales::percent(total / 2516575, accuracy = .1, trim = FALSE)) |>
    dplyr::arrange(desc(perc2))


bg <- "white"
txt_col <- "black"

sysfonts::font_add(family = "Font Awesome 6 Brands", regular = "C:/Users/italo/AppData/Local/Microsoft/Windows/Fonts/Font Awesome 6 Brands-Regular-400.otf") # nolint
showtext::showtext_auto()
showtext::showtext_opts(dpi = 300)

github_icon <- "&#xf09b"
linkedin_icon <- "&#xf0e1"
x_icon <- "&#xf099"
instagram_icon <- "&#xf16d"
github_username <- "italomarquesmonteiro"
linkedin_username <- "italomarquesmonteiro"
x_username <- "italommonteiro"
instagram_username <- "italo.m.m"

colors <- "grey30"
title_text <- glue::glue('Participação de <span style="color:{colors}">**vacas**</span> no abate nacional') # nolint
subtitle_text <- glue::glue("")
caption_text <- glue::glue(
  "**Dados:**  IBGE [Diretoria de Pesquisas Agropecuárias, Coordenação de Agropecuária, Pesquisa Trimestral do Abate de Animais(2024)]<br>", # nolint
  "**Nota:** As unidades da federação, Amapá, Distritio Federal e Paraíba não apresentaram dados<br>",
  "**Plot:** Ítalo Marques-Monteiro <br><br>",
  "<span style='font-family:\"Font Awesome 6 Brands\"; color: black;'>{github_icon};</span> 
  <span style='color: black'>{github_username}</span><br>",
  "<span style='font-family:\"Font Awesome 6 Brands\"; color: dodgerblue4;'>{linkedin_icon};</span> 
  <span style='color: black'>{linkedin_username}</span><br>",
  "<span style='font-family:\"Font Awesome 6 Brands\"; color: steelblue;'>{x_icon};</span>
  <span style='color: black'>{x_username}</span><br>",
  "<span style='font-family:\"Font Awesome 6 Brands\"; color: red;'>{instagram_icon};</span>
  <span style='color: black'>{instagram_username}</span>"
)



perc_abate_vaca <- prop_vaca |>
    dplyr::mutate(
        color = case_when(
          row_number() == 1 ~ "goldenrod1",
          row_number() == 2 ~ "mediumpurple1",
          row_number() == 3 ~ "coral2",
          ## all others should be gray
          TRUE ~ "gray70"),
        color = forcats::as_factor(color)
    ) |>
    ggplot(aes(x = perc, y = reorder(uf, perc))) +
        geom_col(aes(fill = color)) +
        scale_fill_identity(guide = "none") +
        geom_text(mapping = aes(label = scales::percent(perc, accuracy = 0.1)),
             position = position_dodge(0.9),
             size = 3.5,
             color = "gray20",
             vjust = 0.5, 
             hjust = 0.01) +
        theme_void() +
        labs(
            title = title_text,
            subtitle = "4º trimestre de 2023",
            caption = caption_text) +
        theme(
            plot.title = ggtext::element_markdown(color = "gray30", face = "bold", size = 25, hjust = 0.1),
            plot.subtitle = element_text(family = "italic", size = 15, color = "gray50", hjust = 0.1),
            plot.caption = ggtext::element_markdown(hjust = 0, margin = margin(10,0,0,0), size = 8, color = txt_col, lineheight = 1.2), # nolint
            axis.text.y = element_text(color = "gray40", size = 12, hjust = 1, family = "Fira Sans"),
            plot.margin = margin(rep(15, 4)),
            panel.background = element_rect(fill = "white", color = "white"),
            plot.background = element_rect(fill = "white")
        )
#perc_abate_vaca


abt_vaca <- data.frame(image = ".vscode/Images/mapa_abt_vaca.png") 

perc_abate_vaca <- perc_abate_vaca +
    ggimage::geom_image(aes(x = 0.10, y = 10, image = image), data = abt_vaca,  size = 1)  # nolint


ggsave(".vscode/Images/perc_abate_vaca.png", plot = perc_abate_vaca, dpi = 300)
```



Participação de bois no abate nacional

```{r}
ufs_boi <- ufs |>
    dplyr::filter(categoria == "Bois") |>
    dplyr::mutate(uf = as.factor(uf)) |>
    dplyr::glimpse()

ufs_boi |>
    dplyr::summarize(
        total_boi = sum(valor, na.rm = TRUE))


prop_boi <- ufs_boi |>
    dplyr::group_by(uf) |>
    dplyr::summarize(
        total_boi = sum(valor, na.rm = TRUE),
        perc_boi = total_boi / 5115884) |>
    dplyr::mutate(percentual_boi = scales::percent(total_boi / 5115884, accuracy = .1, trim = FALSE))

prop_boi_cor <- prop_boi |>
    dplyr::arrange(desc(perc_boi)) |>
  mutate(
    color = case_when(
      row_number() == 1 ~ "goldenrod1",
      row_number() == 2 ~ "mediumpurple1",
      row_number() == 3 ~ "coral2",
      ## all others should be gray
      TRUE ~ "gray70"
    )
  )


bg <- "white"
txt_col <- "black"

sysfonts::font_add(family = "Font Awesome 6 Brands", regular = "C:/Users/italo/AppData/Local/Microsoft/Windows/Fonts/Font Awesome 6 Brands-Regular-400.otf") # nolint
showtext::showtext_auto()
showtext::showtext_opts(dpi = 300)

github_icon <- "&#xf09b"
linkedin_icon <- "&#xf0e1"
x_icon <- "&#xf099"
instagram_icon <- "&#xf16d"
github_username <- "italomarquesmonteiro"
linkedin_username <- "italomarquesmonteiro"
x_username <- "italommonteiro"
instagram_username <- "italo.m.m"

colors <- "grey30"
title_text <- glue::glue('Participação de <span style="color:{colors}">**bois**</span> no abate nacional') # nolint
subtitle_text <- glue::glue("")
caption_text <- glue::glue(
  "**Dados:**  IBGE [Diretoria de Pesquisas Agropecuárias, Coordenação de Agropecuária, Pesquisa Trimestral do Abate de Animais(2024)]<br>", # nolint
  "**Nota:** As unidades da federação, Amapá, Distritio Federal e Paraíba não apresentaram dados<br>",
  "**Plot:** Ítalo Marques-Monteiro <br><br>",
  "<span style='font-family:\"Font Awesome 6 Brands\"; color: black;'>{github_icon};</span> 
  <span style='color: black'>{github_username}</span><br>",
  "<span style='font-family:\"Font Awesome 6 Brands\"; color: dodgerblue4;'>{linkedin_icon};</span> 
  <span style='color: black'>{linkedin_username}</span><br>",
  "<span style='font-family:\"Font Awesome 6 Brands\"; color: steelblue;'>{x_icon};</span>
  <span style='color: black'>{x_username}</span><br>",
  "<span style='font-family:\"Font Awesome 6 Brands\"; color: red;'>{instagram_icon};</span>
  <span style='color: black'>{instagram_username}</span>"
)



perc_abt_boi <- prop_boi_cor |>
    ggplot(aes(x = perc_boi, y = reorder(uf, perc_boi))) +
        geom_col(aes(fill = color))+
        scale_fill_identity(guide = "none") +
        geom_text(mapping = aes(label = scales::percent(perc_boi, accuracy = 0.1)),
             position = position_dodge(0.9),
              size = 3.5,
              color = "gray20",
             vjust = 0.5, 
             hjust = 0.01) +
        theme_void() +
        labs(
            title = title_text,
            subtitle = "4º trimestre de 2023",
            caption = caption_text) +
        theme(
            plot.title = ggtext::element_markdown(color = "gray30", face = "bold", size = 25, hjust = 0.1),
            plot.subtitle = element_text(family = "italic", size = 15, color = "gray50", hjust = 0.1),
            plot.caption = ggtext::element_markdown(hjust = 0, margin = margin(10,0,0,0), size = 8, color = txt_col, lineheight = 1.2), # nolint
            axis.text.y = element_text(color = "gray40", size = 12, hjust = 1, family = "Fira Sans"),
            plot.margin = margin(rep(15, 4)),
            panel.background = element_rect(fill = "white", color = "white"),
            plot.background = element_rect(fill = "white")
        )
#perc_abt_boi

abt_boi <- data.frame(image = ".vscode/Images/mapa_abt_boi.png") 

perc_abt_boi <- perc_abt_boi +
    ggimage::geom_image(aes(x = 0.10, y = 10, image = image), data = abt_boi,  size = 1)  # nolint

ggsave(".vscode/Images/perc_abt_boi.png", plot = perc_abt_boi, dpi = 300)
```




Participação de novilhas no abate nacional

```{r}
ufs_novilha <- ufs |>
    dplyr::filter(categoria == "Novilhas") |>
    dplyr::mutate(uf = as.factor(uf)) |>
    dplyr::glimpse()

ufs_novilha |>
    dplyr::summarize(
        total_novilha = sum(valor, na.rm = TRUE))


prop_novilha <- ufs_novilha |>
    dplyr::group_by(uf) |>
    dplyr::summarize(
        total_novilha = sum(valor, na.rm = TRUE),
        perc_novilha = total_novilha / 1027576) |>
    dplyr::mutate(percentual_novilha = scales::percent(total_novilha / 1027576, accuracy = .1, trim = FALSE))

prop_novilha_cor <- prop_novilha |>
    dplyr::arrange(desc(perc_novilha)) |>
  mutate(
    color = case_when(
      row_number() == 1 ~ "goldenrod1",
      row_number() == 2 ~ "mediumpurple1",
      row_number() == 3 ~ "coral2",
      TRUE ~ "gray70"
    )
  )



bg <- "white"
txt_col <- "black"

sysfonts::font_add(family = "Font Awesome 6 Brands", regular = "C:/Users/italo/AppData/Local/Microsoft/Windows/Fonts/Font Awesome 6 Brands-Regular-400.otf") # nolint
showtext::showtext_auto()
showtext::showtext_opts(dpi = 300)

github_icon <- "&#xf09b"
linkedin_icon <- "&#xf0e1"
x_icon <- "&#xf099"
instagram_icon <- "&#xf16d"
github_username <- "italomarquesmonteiro"
linkedin_username <- "italomarquesmonteiro"
x_username <- "italommonteiro"
instagram_username <- "italo.m.m"

colors <- "grey30"
title_text <- glue::glue('Participação de <span style="color:{colors}">**novilhas**</span> no abate nacional') # nolint
subtitle_text <- glue::glue("")
caption_text <- glue::glue(
  "**Dados:**  IBGE [Diretoria de Pesquisas Agropecuárias, Coordenação de Agropecuária, Pesquisa Trimestral do Abate de Animais(2024)]<br>", # nolint
  "**Nota:** As unidades da federação, Amapá, Distritio Federal e Paraíba não apresentaram dados<br>",
  "**Plot:** Ítalo Marques-Monteiro <br><br>",
  "<span style='font-family:\"Font Awesome 6 Brands\"; color: black;'>{github_icon};</span> 
  <span style='color: black'>{github_username}</span><br>",
  "<span style='font-family:\"Font Awesome 6 Brands\"; color: dodgerblue4;'>{linkedin_icon};</span> 
  <span style='color: black'>{linkedin_username}</span><br>",
  "<span style='font-family:\"Font Awesome 6 Brands\"; color: steelblue;'>{x_icon};</span>
  <span style='color: black'>{x_username}</span><br>",
  "<span style='font-family:\"Font Awesome 6 Brands\"; color: red;'>{instagram_icon};</span>
  <span style='color: black'>{instagram_username}</span>"
)

perc_abt_novilha <- prop_novilha_cor |>
    ggplot(aes(x = perc_novilha, y = reorder(uf, perc_novilha))) +
        geom_col(aes(fill = color))+
        scale_fill_identity(guide = "none") +
        geom_text(mapping = aes(label = scales::percent(perc_novilha, accuracy = 0.1)),
             position = position_dodge(0.9), 
             size = 3.5,
             color = "gray20",
             vjust = 0.5, 
             hjust = 0.01) +
        theme_void() +
        labs(
            title = title_text,
            subtitle = "4º trimestre de 2023",
            caption = caption_text) +
        theme(
            plot.title = ggtext::element_markdown(color = "gray30", face = "bold", size = 25, hjust = 0.1),
            plot.subtitle = element_text(family = "italic", size = 15, color = "gray50", hjust = 0.1),
            plot.caption = ggtext::element_markdown(hjust = 0, margin = margin(10,0,0,0), size = 8, color = txt_col, lineheight = 1.2), # nolint
            axis.text.y = element_text(color = "gray40", size = 12, hjust = 1, family = "Fira Sans"),
            plot.margin = margin(rep(15, 4)),
            panel.background = element_rect(fill = "white", color = "white"),
            plot.background = element_rect(fill = "white")
        )
#perc_abt_novilha

abt_novilha <- data.frame(image = ".vscode/Images/mapa_abt_novilha.png") 

perc_abt_novilha <- perc_abt_novilha +
    ggimage::geom_image(aes(x = 0.17, y = 10, image = image), data = abt_novilha,  size = 1)  # nolint

ggsave(".vscode/Images/perc_abt_novilha.png", plot = perc_abt_novilha, dpi = 300)
```


Participação de novilhos no abate nacional

```{r}
ufs_novilho <- ufs |>
    dplyr::filter(categoria == "Novilhos") |>
    dplyr::mutate(uf = as.factor(uf)) |>
    dplyr::glimpse()

ufs_novilho |>
    dplyr::summarize(
        total_novilho = sum(valor, na.rm = TRUE))


prop_novilho <- ufs_novilho |>
    dplyr::group_by(uf) |>
    dplyr::summarize(
        total_novilho = sum(valor, na.rm = TRUE),
        perc_novilho = total_novilho / 372911) |>
    dplyr::mutate(percentual_novilho = scales::percent(total_novilho / 372911, accuracy = .1, trim = FALSE))

prop_novilho_cor <- prop_novilho |>
    dplyr::arrange(desc(perc_novilho)) |>
  mutate(
    color = case_when(
      row_number() == 1 ~ "goldenrod1",
      row_number() == 2 ~ "mediumpurple1",
      row_number() == 3 ~ "coral2",
      TRUE ~ "gray70"
    )
  )



bg <- "white"
txt_col <- "black"

sysfonts::font_add(family = "Font Awesome 6 Brands", regular = "C:/Users/italo/AppData/Local/Microsoft/Windows/Fonts/Font Awesome 6 Brands-Regular-400.otf") # nolint
showtext::showtext_auto()
showtext::showtext_opts(dpi = 300)

github_icon <- "&#xf09b"
linkedin_icon <- "&#xf0e1"
x_icon <- "&#xf099"
instagram_icon <- "&#xf16d"
github_username <- "italomarquesmonteiro"
linkedin_username <- "italomarquesmonteiro"
x_username <- "italommonteiro"
instagram_username <- "italo.m.m"

colors <- "grey30"
title_text <- glue::glue('Participação de <span style="color:{colors}">**novilhos**</span> no abate nacional') # nolint
subtitle_text <- glue::glue("")
caption_text <- glue::glue(
  "**Dados:**  IBGE [Diretoria de Pesquisas Agropecuárias, Coordenação de Agropecuária, Pesquisa Trimestral do Abate de Animais(2024)]<br>", # nolint
  "**Nota:** As unidades da federação, Amapá, Distritio Federal e Paraíba não apresentaram dados<br>",
  "**Plot:** Ítalo Marques-Monteiro <br><br>",
  "<span style='font-family:\"Font Awesome 6 Brands\"; color: black;'>{github_icon};</span> 
  <span style='color: black'>{github_username}</span><br>",
  "<span style='font-family:\"Font Awesome 6 Brands\"; color: dodgerblue4;'>{linkedin_icon};</span> 
  <span style='color: black'>{linkedin_username}</span><br>",
  "<span style='font-family:\"Font Awesome 6 Brands\"; color: steelblue;'>{x_icon};</span>
  <span style='color: black'>{x_username}</span><br>",
  "<span style='font-family:\"Font Awesome 6 Brands\"; color: red;'>{instagram_icon};</span>
  <span style='color: black'>{instagram_username}</span>"
)


perc_abt_novilho <- prop_novilho_cor |>
    ggplot(aes(x = perc_novilho, y = reorder(uf, perc_novilho))) +
        geom_col(aes(fill = color))+
        scale_fill_identity(guide = "none") +
        geom_text(mapping = aes(label = scales::percent(perc_novilho, accuracy = 0.1)),
             position = position_dodge(0.9), 
             size = 3.5,
             color = "gray20",
             vjust = 0.5, 
             hjust = 0.01) +
        theme_void() +
        labs(
            title = title_text,
            subtitle = "4º trimestre de 2023",
            caption = caption_text) +
        theme(
            plot.title = ggtext::element_markdown(color = "gray30", face = "bold", size = 25, hjust = 0.1),
            plot.subtitle = element_text(family = "italic", size = 15, color = "gray50", hjust = 0.1),
            plot.caption = ggtext::element_markdown(hjust = 0, margin = margin(10,0,0,0), size = 8, color = txt_col, lineheight = 1.2), # nolint
            axis.text.y = element_text(color = "gray40", size = 12, hjust = 1, family = "Fira Sans"),
            plot.margin = margin(rep(15, 4)),
            panel.background = element_rect(fill = "white", color = "white"),
            plot.background = element_rect(fill = "white")
        )
#perc_abt_novilho

abt_novilho <- data.frame(image = ".vscode/Images/mapa_abt_novilho.png") 

perc_abt_novilho <- perc_abt_novilho +
    ggimage::geom_image(aes(x = 0.20, y = 10, image = image), data = abt_novilho,  size = 1)  # nolint
plotly::ggplotly(perc_abt_novilho)

ggsave(".vscode/Images/perc_abt_novilho.png", plot = perc_abt_novilho, dpi = 300)
```


mapas 
```{r}
br <- geobr::read_state(code_state = "all", year = 2020) |>
    dplyr::glimpse()

rs <- br |> dplyr::filter(abbrev_state == "RS")
ms <- br |> dplyr::filter(abbrev_state == "MS") 
go <- br |> dplyr::filter(abbrev_state == "GO") 
sp <- br |> dplyr::filter(abbrev_state == "SP") 
mt <- br |> dplyr::filter(abbrev_state == "MT") 
pa <- br |> dplyr::filter(abbrev_state == "PA") 
ro <- br |> dplyr::filter(abbrev_state == "RO")
sc <- br |> dplyr::filter(abbrev_state == "SC")


mapa_abt_novilho <- br |>
    ggplot2::ggplot() +
        geom_sf(
            fill = "gray70",
            color = "gray30",
            size = .15,
            show.legend = FALSE) +
        geom_sf(data = rs,
        
            fill = "goldenrod1",
            color = "gray30",
            size = .15,
            show.legend = FALSE) +
        geom_sf(data = ms,
            fill = "mediumpurple1",
            color = "gray30",
            size = .15,
            show.legend = FALSE) +
        geom_sf(data = sc,
            fill = "coral2",
            color = "gray30",
            size = .15,
            show.legend = FALSE) +
        theme_void() +
        theme(
            panel.background = element_blank(),
            plot.background = element_blank()
        )
mapa_abt_novilho
ggsave(".vscode/Images/mapa_abt_novilho.png", plot = mapa_abt_novilho, dpi = 300)
```