...
percentual_abates |>
    ...
    ggplot(
        mapping = aes(
            x = trimestre,
                y = ifelse(
                    sexo == "Fêmea",  yes = perc_categoria, no = -perc_categoria), fill = sexo)) + # nolint
      geom_bar(stat = "identity") +
      scale_y_continuous(breaks = seq(-75, 75, 10), labels = abs(seq(-75, 75, 0))) + # nolint
      #scale_y_continuous(labels = abs_virgula, limits = (max(piramide$pop))* c(-1,1)) + # nolint
