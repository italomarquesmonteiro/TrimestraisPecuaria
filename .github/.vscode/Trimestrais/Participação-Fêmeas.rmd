...
percentual_abate_trimestral_categorias <- abt |>
  #dplyr::filter(ano >= 2010) |>
  group_by(ano, trimestre, categoria, sexo) |>
  summarise(
    total_cabeca = sum(cabeca)#,.groups = 'drop'
  ) |>
...
#percentual_abate_trimestral_categorias <- percentual_abate_trimestral_categorias |>
...
# Ajustando a ordem dos níveis de 'categoria'
percentual_abate_trimestral_categorias <- percentual_abate_trimestral_categorias |>
  mutate(
    categoria = factor(categoria, levels = c("Novilhas", "Vacas"))
  )
...
txt_col <- "grey70"
colors <- "gray40"
title_text3 <- glue::glue("**Abate fêmeas:**")
subtitle_text3 <- glue::glue("**Participação de vacas e novilhas frente ao total abatido por trimestre (1997 - 2024)**")
caption_text3 <- glue::glue(
  ...
  "**Nota:** As linhas pontilhadas indicam o valor do último percentual de abate das categorias em relação à série.<br>",
...
pal <- colorRampPalette(cores_base)(2)
#cor <- c("#ff7c43", "#ffa600")
cor <- c("#665191", "#a05195")


# Gerando o gráfico mde barras empilhadas com facet
plot_percentual_abate_trimestral_categorias <- percentual_abate_trimestral_categorias |>
  #dplyr::filter(categoria == "Novilhas") |> arrange(desc(percentual))
  dplyr::filter(sexo == "Fêmea") |>  #arrange(desc(percentual))
  ggplot(aes(x = trimestre, y = percentual, fill = categoria)) +  # Removido fill = categoria
  ggchicklet::geom_chicklet(alpha = 1, color = "grey50", size = 0.8) +
  geom_hline(yintercept = 14.5, color = "#fd001e", lty = 3) +
  geom_hline(yintercept = 45.3, color = "#53c4f1", lty = 3) +
  scale_y_continuous(labels = scales::label_percent(scale = 1), position = "right") +  # Eixo y no lado direito e com %
  scale_fill_manual(values = c("#fd001e", "#53c4f1")) +
  #scale_fill_manual(values = c("#fcaf45", "#f77737")) +
  scale_x_discrete(
    limits = c(
  ...
  labs(
    title = title_text3,
    subtitle = subtitle_text3,
    caption = caption_text3,
    fill = ""
  ) +
  theme(
    plot.title = ggtext::element_markdown(face = "italic", family = font3, size = 40, color = "gray80", hjust = 0.5),
    plot.subtitle = ggtext::element_markdown(face = "italic", family = font2, size = 18, color = "gray70", hjust = 0.5),
    ...
    legend.key = element_rect(fill = "grey25", color = "grey25"),
    legend.text = element_text(face = "bold", color = "grey60", size = 14),
    legend.title = element_text(face = "bold", color = "grey60", size = 14),
    axis.title.x = element_blank(),
    axis.title.y = element_blank(),
    axis.text.y = ggtext::element_markdown(face = "bold", family = font2, size = 8, color = "gray50"),
    axis.text.x = ggtext::element_markdown(face = "bold", family = font2, size = 8, color = "gray50", angle = 90, hjust = 1, vjust = 0.5),
    axis.ticks.length.x = unit(-0.8, "cm"),
    axis.ticks.x = element_line(color = "grey50", size = 0.8)
  )

ggsave(".github\\.vscode\\Trimestrais\\Percentual de participação trimestral - Vacas e Novilhas.png",
       plot_percentual_abate_trimestral_categorias,
       width = 17.1,
       height = 10.2,
       #units = "cm",
       dpi = 300)
