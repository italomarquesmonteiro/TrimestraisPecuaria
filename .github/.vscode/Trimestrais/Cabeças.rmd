...
```{r}
# Criando uma nova coluna para identificar machos e fêmeas
abt <- abt |>
  mutate(
    sexo = case_when(
...
# Calculando o total de cabeças por gênero e o percentual
percentual_abate <- abt |>
  group_by(ano, sexo) |>
  summarise(
...
print(percentual_abate) |> tail(10)
...
percentual_abate |>
  ...
  ggchicklet::geom_chicklet(position = "dodge", width = 0.7) +  # Barras arredondadas
  geom_text(aes(label = percentual), position = position_dodge(width = 0.7), vjust = -0.5) +
  scale_fill_manual(values = pal) +  # Aplicar a paleta de cores
  labs(
    title = "Percentual de Abate de Fêmeas por Ano",
    x = "Ano",
    y = "Percentual de Fêmeas Abatidas (%)",
    fill = "Ano"
  ) +
  theme(
...
txt_col <- "grey70"
colors <- "gray40"
title_text <- glue::glue("Percentual de Abate de Fêmeas por Ano")
subtitle_text <- glue::glue("Participação anual de fêmeas sobre o total abatido")
...
# Criando o gráfico com percentual de fêmeas abatidas
part_abt_femeas <- percentual_abate |>
  dplyr::filter(sexo == "Fêmea") |>  # Filtrar apenas os dados de fêmeas
  ggplot(aes(x = as.factor(ano), y = percentual, fill = as.factor(ano))) +  # Criar o gráfico de barras
  ggchicklet::geom_chicklet(position = "dodge", width = 0.7) +  # Barras arredondadas
  geom_text(aes(label = sprintf("%.1f", percentual)), position = position_dodge(width = 0.7), vjust = -0.5) +  # Adicionar labels com uma casa decimal
  scale_fill_manual(values = pal) +  # Aplicar a paleta de cores
  labs(
    title = "Percentual de Abate de Fêmeas por Ano",
    x = "Ano",
