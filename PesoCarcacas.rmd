...
peso_medio_carcaca_2tri_2023 <- abate |>
    dplyr::filter(tipo_de_rebanho == "Bovinos") |>
    #dplyr::summarise(media = mean(peso_medio_carcaca)) |>
    ggplot2::ggplot(aes(x = trimestre, y = peso_medio_carcaca)) +
                geom_col(aes(fill = ifelse(
                    trimestre %in% c(
                        "2º trimestre 2022",
                            "1º trimestre 2023",
  ...
  scale_fill_manual(values = c(high = "#346116", default = "grey45")) +
                geom_hline(yintercept = 267., lty = 3, color = "gray30") +
                theme_swd_line  +
                scale_y_continuous(labels = scales::comma) +
                labs(title = "Abate de bovinos no Brasil (Peso medio da carcaça - kg)",
                    subtitle = "# Relatório trimestral de abate
                    by Ítalo Monteiro
                    Source: IBGE [open data(2023)]",
                    x = "Trimestre"
